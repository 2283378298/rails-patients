class <%= controller_class_name %>Controller < ApplicationController
  ...
  def index
    @q = current_shop.<%= plural_name %>.ransack(params[:q])
  ...
  def new
    @<%= singular_name %> = current_shop.<%= plural_name %>.build
    respond_to do |format|
      format.html
      format.js
    end
  ...
  def create
    @<%= singular_name %> = current_shop.<%= plural_name %>.build(<%= singular_name %>_params)
  ...
  def set_<%= singular_name %>
    @<%= singular_name %> = current_shop.<%= plural_name %>.find(params[:id])
  end
